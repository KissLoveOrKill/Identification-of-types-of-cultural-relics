...
首先，请从以下链接下载本项目所需的数据集：

> **数据集下载**
> *   **文件名**: `Antique.zip`
> *   **下载链接**: [百度网盘](https://pan.baidu.com/s/1pFX3kVi1mdt0XJ9hnxDNTg)
> *   **提取码**: `bx3e`

下载完成后，请确保将数据集文件（`Antique.zip`）放置在与你要运行的模型脚本（`.py` 文件）**相同的目录**下。

> **数据集结构说明 (`Antique.zip` 或解压后的文件夹)**
>
> 数据集应符合 `torchvision.datasets.ImageFolder` 的标准格式，即包含以类别名称命名的子文件夹，每个子文件夹中存放对应类别的文物图片。
>
> 示例结构：
> ```text
> data/ (自动解压后的目录)
...
> └── ...
> ```
> 此项目代码会自动查找并解压同目录下的 `.zip` 文件到 `data` 文件夹用于训练。
